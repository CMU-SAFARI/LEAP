...
__m256i shift_right_avx(__m256i vec, int shift_num) {
	if (shift_num >= 128) {
		vec = _mm256_setr_m128i(_mm_setzero_ps(), _mm256_exracti128_si256(vec, 0));
		vec = shift_num % 128;
	}
	if (shift_num >= 64) {
...
__m256i shift_left_avx(__m256i vec, int shift_num) {
	if (shift_num >= 128) {
		vec = _mm256_setr_m128i(_mm256_exracti128_si256(vec, 1), _mm_setzero_ps());
		vec = shift_num % 128;
	}
	if (shift_num >= 64) {
		vec = _mm_srli_si256(vec, 8);
		shift_num = shift_num % 64;
	}
	__m256i carryover = _mm_srli_si256(vec, 8);
	carryover = _mm_slli_epi64(carryover, 64 - shift_num);
	vec = _mm_srli_epi64(vec, shift_num);
	return _mm_or_si256(vec, carryover);
}
